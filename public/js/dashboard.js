...
(function () {
    ...
    function processDynamicComponents(container) {
        if (!container) return;
        container.querySelectorAll('[data-dynamic-width]').forEach(el => {
            const bar = el.querySelector('.confidence-bar');
            if (bar) {
                bar.style.width = el.dataset.dynamicWidth + '%';
                if (el.dataset.dynamicColor) bar.style.background = el.dataset.dynamicColor;
            }
        });
    }
    ...
    function confidenceBar(val) {
        ...
        return `<div class="confidence-bar-container" data-dynamic-width="${val}" data-dynamic-color="${color}"><div class="confidence-bar"></div><span class="confidence-text">${val}%</span></div>`;
    }
        ...
        try {
            ...
            if (d.recentScans && d.recentScans.length > 0) {
                ...
                tbody.innerHTML = d.recentScans.map(s => `<tr>
                    ...
                    <td><button class="btn btn-outline btn-sm" data-action="viewScanDetail" data-arg0="${s.id}" aria-label="Details anzeigen"><i class="bi bi-eye"></i></button></td>
                </tr>`).join('');
            } else {
            ...
            if (results.length > 0) {
                ...
                for (const [ip, ports] of Object.entries(grouped)) {
                    ...
                    html += ports.map(r => {
                        let svcInfo = esc(r.service || '-');
                        if (r.banner) {
                            svcInfo = `<strong>${esc(r.service || '-')}</strong><br><span class="fs-08 color-secondary d-inline-block max-w-300 text-truncate" title="${esc(r.banner)}">${esc(r.banner)}</span>`;
                        } else if (r.service_product) {
                            let ver = r.service_product;
                            if (r.service_version) ver += ' ' + r.service_version;
                            svcInfo = `<strong>${esc(r.service || '-')}</strong><br><span class="fs-08 color-secondary">${esc(ver)}</span>`;
        ...
        try {
            ...
            if (d.scans && d.scans.length > 0) {
                ...
                tbody.innerHTML = d.scans.map(s => `<tr>
                    ...
                    <td>${statusBadge(s.status)}</td><td>${s.result_count || 0}</td><td>${s.vuln_count || 0}</td>
                    <td>${formatDate(s.started_at)}</td>
                    <td><div class="d-flex gap-1"><button class="btn btn-outline btn-sm" data-action="viewScanDetail" data-arg0="${s.id}" aria-label="Details anzeigen"><i class="bi bi-eye"></i></button>
                    <button class="btn btn-danger btn-sm" data-action="deleteScan" data-arg0="${s.id}" aria-label="Scan löschen"><i class="bi bi-trash"></i></button></div></td>
                </tr>`).join('');
            } else {
                empty.classList.remove('hidden');
            }
            ...
            if (schedules.length > 0) {
                tbody.innerHTML = schedules.map(s => `<tr>
                    ...
                    <td><div class="d-flex gap-1">
                        <button class="btn btn-outline btn-sm" data-action="toggleSchedule" data-arg0="${s.id}" data-arg1="${!s.enabled}" aria-label="${s.enabled ? 'Zeitplan pausieren' : 'Zeitplan aktivieren'}"><i class="bi bi-${s.enabled ? 'pause' : 'play'}"></i></button>
                        <button class="btn btn-danger btn-sm" data-action="deleteSchedule" data-arg0="${s.id}" aria-label="Zeitplan löschen"><i class="bi bi-trash"></i></button>
                    </div></td>
                </tr>`).join('');
            } else {
    ...
    async function loadUsers() {
        try {
            ...
            tbody.innerHTML = users.map(u => `<tr>
                ...
                <td><button class="btn btn-danger btn-sm" data-action="deleteUser" data-arg0="${u.id}" ${u.id === currentUser?.id ? 'disabled' : ''} aria-label="Benutzer löschen"><i class="bi bi-trash"></i></button></td>
            </tr>`).join('');
        } catch (e) { showToast('error', 'Fehler', e.message); }
    }
                ...
                tbody.innerHTML = fps.map(f => `<tr>
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            <button class="btn btn-outline btn-sm" data-action="showFingerprintDetail" data-arg0="${f.id}" title="Details" aria-label="Details anzeigen"><i class="bi bi-eye"></i></button>
                            <button class="btn btn-danger btn-sm" data-action="deleteFingerprint" data-arg0="${f.id}" title="Löschen" aria-label="Fingerprint löschen"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>`).join(''); processDynamicComponents(tbody);
            } else {
    ...
    window.showFingerprintDetail = async function (id) {
        try {
            ...
            document.getElementById('fpDetailContent').innerHTML = `
                ...
                </div>`; processDynamicComponents(document.getElementById('fpDetailContent'));
        ...
        try {
            ...
            if (exps.length > 0) {
                tbody.innerHTML = exps.map(ex => {
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            <button class="btn btn-outline btn-sm" data-action="showExploitDetail" data-arg0="${ex.id}" title="Details" aria-label="Details anzeigen"><i class="bi bi-eye"></i></button>
                            ${ex.exploit_code ? `<button class="btn btn-outline btn-sm" data-action="showExploitCode" data-arg0="${ex.id}" title="Code anzeigen" aria-label="Code anzeigen" data-exploit-id="${ex.id}" data-edb-id="${esc(ex.exploit_db_id || '')}"><i class="bi bi-code-slash"></i></button>` : ''}
                            ${ex.source_url ? `<a href="${esc(ex.source_url)}" target="_blank" class="btn btn-outline btn-sm" title="Referenz" aria-label="Referenz öffnen"><i class="bi bi-box-arrow-up-right"></i></a>` : ''}
                            <button class="btn btn-danger btn-sm" data-action="deleteExploit" data-arg0="${ex.id}" title="Löschen" aria-label="Exploit löschen"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>`;
                }).join('');
    ...
    async function loadCVEs(page) {
        ...
        try {
            ...
            if (cves.length > 0) {
                tbody.innerHTML = cves.map(c => `<tr class="cursor-pointer" data-action="showCVEDetail" data-arg0="${esc(c.cve_id)}">
    ...
    window.showAutoAttackModal = async function(scanId, targetIp) {
        try {
            ...
            content.innerHTML = '<div class="text-center" class="p-2"><div class="spinner"></div><p class="text-muted mt-1">Analysiere Dienste und suche passende Exploits...</p></div>';
            ...
            if (attackable.length === 0) {
                content.innerHTML = `
                    <div class="text-center" class="p-15">
                        <i class="bi bi-shield-check" class="fs-3 color-green"></i>
                        ...
                        <div class="mt-2" class="text-left">
                            <h5 class="mb-1">Erkannte Dienste (keine Exploits verfügbar):</h5>
                            ${notAttackable.map(s => `
                                <div class="d-flex align-center gap-05 p-04-0 border-bottom">
            ...
            content.innerHTML = `
                <div class="mb-1">
                    <div class="d-flex align-center gap-05 mb-05">
                        <i class="bi bi-crosshair" class="fs-15 color-red"></i>
                ...
                <div class="border-red-3 radius-sm overflow-hidden mb-1">
                    ${attackable.map(s => {
                        const sevColors = { critical: 'red', high: 'orange', medium: 'yellow', low: 'blue' };
                        const topSev = (s.severities || '').split(',')[0] || 'medium';
                        return `<div class="d-flex align-center justify-between p-06-08 border-bottom bg-red-005">
                ...
                ${notAttackable.length > 0 ? `
                <details class="mb-1">
                    <summary class="text-muted" class="cursor-pointer fs-085"><i class="bi bi-shield-check mr-1"></i>${notAttackable.length} sichere Dienste (keine Exploits)</summary>
                    <div class="mt-05">
                        ${notAttackable.map(s => `
                            <div class="d-flex align-center gap-05 p-03-0 fs-085">
                                <span class="badge badge-green" class="fs-07">OK</span>
                ...
                <div class="bg-tertiary radius-sm p-08 fs-085">
                    <div class="d-flex align-center gap-05 mb-05">
    ...
    window.startAutoAttack = async function() {
        ...
        startBtn.disabled = true;
        startBtn.innerHTML = '<span class="spinner" class="spinner-sm"></span> Angriff läuft...';
        progressDiv.classList.remove('hidden');
        progressDiv.innerHTML = `
            <div class="p-1 bg-tertiary radius-sm mt-1">
                <div class="d-flex align-center gap-05 mb-05">
                    <span class="spinner" class="spinner-md"></span>
        ...
        try {
            ...
            if (d.status === 'no_exploits' || d.status === 'no_executable_exploits') {
                progressDiv.innerHTML = `
                    <div class="p-1 bg-green-1 border-green-2 radius-sm mt-1 text-center">
                        <i class="bi bi-shield-check" class="fs-2 color-green"></i>
            ...
            const pollInterval = setInterval(async () => {
                ...
                try {
                    ...
                    if (pollCount >= maxPolls) {
                        clearInterval(pollInterval);
                        progressDiv.innerHTML = '<p class="text-muted" class="p-1">Zeitüberschreitung. Prüfen Sie die Ergebnisse in der Angriffsketten-Übersicht.</p>';
                        startBtn.innerHTML = '<i class="bi bi-lightning-charge-fill"></i> Angriff starten';
                        startBtn.disabled = false;
                    }
                }
            ...
            showToast('error', 'Fehler', e.message);
            progressDiv.innerHTML = `<div class="p-1 color-red"><i class="bi bi-x-circle"></i> ${esc(e.message)}</div>`;
        ...
        if (shellFinding) {
            ...
            html += `
                <div class="p-15 bg-green-1 border-green-4 radius-sm mt-1 text-center">
                    <i class="bi bi-terminal-fill" class="fs-3 color-green"></i>
                    <h3 class="color-green m-05-0">Reverse Shell erhalten!</h3>
            ...
            html += `
                <div class="p-15 bg-yellow-1 border-yellow-3 radius-sm mt-1 text-center">
                    <i class="bi bi-shield-exclamation" class="fs-3 color-yellow"></i>
                    <h3 class="color-yellow m-05-0">Angriff abgeschlossen</h3>
            ...
            html += `
                <div class="p-15 bg-red-1 border-red-3 radius-sm mt-1 text-center">
                    <i class="bi bi-x-circle" class="fs-3 color-red"></i>
                    <h3 class="color-red m-05-0">Angriff fehlgeschlagen</h3>
        ...
        if (results.length > 0) {
            html += `
                <details class="mt-1" ${shellFinding ? '' : 'open'}>
                    <summary class="cursor-pointer fw-600 p-05-0"><i class="bi bi-list-check mr-1"></i> Detaillierte Ergebnisse (${results.length} Schritte)</summary>
                    <div class="mt-05">
                        ${results.map((r, i) => {
                            ...
                            return `<div class="flex gap-05 p-05-0 border-bottom">
                                <i class="bi ${icon}" class="mt-2px ${r.status === 'completed' ? 'color-green' : r.status === 'failed' ? 'color-red' : 'color-yellow'}"></i>
                                <div class="flex-1">
                    ...
                    return `<div class="chain-card">
                        ...
                        <div class="chain-footer">
                            ...
                            <div class="d-flex gap-1">
                                ...
                                <button class="btn btn-danger btn-sm" data-action="deleteChain" data-arg0="${c.id}" aria-label="Kette löschen"><i class="bi bi-trash"></i></button>
                            </div>
                        </div>
                    </div>`;
                }).join('');
        ...
        try {
            ...
            if (execs.length > 0) {
                tbody.innerHTML = execs.map(ex => `<tr>
                    ...
                    <td><button class="btn btn-outline btn-sm" data-action="showChainExecDetail" data-arg0="${ex.id}" aria-label="Details anzeigen"><i class="bi bi-eye"></i></button></td>
                </tr>`).join('');
            } else {
                    ...
                    return `<tr>
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                ...
                                <button class="btn btn-danger btn-sm" data-action="deleteAudit" data-arg0="${a.id}" aria-label="Audit löschen"><i class="bi bi-trash"></i></button>
                            </div>
                        </td>
                    </tr>`;
                }).join('');
        ...
        try {
            ...
            if (entries.length > 0) {
                tbody.innerHTML = entries.map(e => `<tr>
                    ...
                    <td><code class="text-sm" class="d-block max-w-400 text-truncate" title="${esc(e.query)}">${esc(e.query)}</code></td>
                    <td>${esc((e.short_description || e.textual_description || '').substring(0, 80))}</td>
                    <td>${esc(e.date || '-')}</td>
                    <td>
                        <a href="https://www.google.com/search?q=${encodeURIComponent(e.query)}" target="_blank" class="btn btn-outline btn-sm" title="Auf Google suchen" aria-label="Auf Google suchen"><i class="bi bi-google"></i></a>
                    </td>
                </tr>`).join('');
                if (emptyEl) emptyEl.classList.add('hidden');
            } else {
